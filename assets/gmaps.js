...
function mapInit() {
    function getDirections(element, coords) {
        ...
        element.directionsService.route(element.directionOptions, function (response) {
            if (response.routes.length){
                console.table(response.routes[0].legs[0].steps)
                $(element.dataset.duration_holder).html(response.routes[0].legs[0].duration.text);
                $(element.dataset.distance_holder).html(response.routes[0].legs[0].distance.text);
            }
            element.bounds = new google.maps.LatLngBounds;
            element.bounds.extend(element.places[0].marker.position);
            element.bounds.extend(new google.maps.LatLng(coords.latitude, coords.longitude));
    ...
    "undefined" == typeof window.mapstyles && (window.mapstyles = "");
    "undefined" == typeof window.defaultzoom && (window.defaultzoom = 14);
    Array.prototype.forEach.call(document.getElementsByClassName("map-canvas"), function (element) {
        element.createMap = function(){
            ...
            function addToMap(geoloc, coords) {
                for (i = 0; i < element.places.length; i++) {
                    if (geoloc) {
                        element.places[i].distance = distance(coords.latitude, coords.longitude, element.places[i].latitude, element.places[i].longitude);
                        console.log(element.places[i].distance)
                    }
